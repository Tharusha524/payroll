...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_product') {
            ...
            if ($pid > 0) {
                $db->execute(
                    'UPDATE products SET name=?, target_weekday=?, target_saturday=?,
                            rate_above=?, rate_below=?, sort_order=?
                      WHERE id=?',
                    [$name, $targetWd, $targetSat, $rateAbove, $rateBelow, $sortOrder, $pid]
                );
        ...
        $pid = postInt('product_id');
        if ($pid > 0) {
            $name = $db->fetchOne('SELECT name FROM products WHERE id=?', [$pid]);
            $db->execute('UPDATE products SET is_active=0 WHERE id=?', [$pid]);
            AuditLog::write('delete', 'products', $pid, "Deactivated product: " . ($name['name'] ?? $pid));
...
$products = $db->fetchAll('SELECT * FROM products WHERE is_active=1 ORDER BY sort_order, id');
                ...
                <table class="pp-table">
                    ...
                    <?php foreach ($products as $p): ?>
                        <tr>
                            ...
                            <td>
                                <form method="POST" onsubmit="return confirmProductDelete(event, '<?= addslashes($p['name']) ?>');" style="display:inline;">
                                    <?= csrfField() ?>
...
<!-- SweetAlert2 (used only on this page) -->
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
function confirmProductDelete(e, name) {
    e.preventDefault();
    const form = e.target;
    Swal.fire({
...
function openForm(id, name, twd, tsat, above, below, sort) {
